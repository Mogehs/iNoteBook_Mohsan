...
const register = async (req, res) => {
  try {
    ...
    const CreatedUser = await User.create({ username, email,password:hash_pass, phone });
    if (CreatedUser) {
      res.status(200).json({ massage:CreatedUser, token: await CreatedUser.generateToken() });}
...
const login = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({ message: "Login successful", user: isUser, token }); 
  } catch (error) {
    next(error); 
  }
};
